...
// CHANGELOG v0.00.09
// ─────────────────────────────────────────────────────────────
// NEW: HLS playback via hls.js (CDN loaded on demand).
//      PeerTube HLS-only results now play instead of erroring.
//      cmdPick/cmdPt: try WebTorrent files first, fall back to
//      HLS m3u8 if no WebTorrent files exist on that instance.
//      !hs url now auto-detects .m3u8 URLs and plays via HLS.
//      HLS streams show [HLS] in the HUD — no P2P seeding
//      (HLS is HTTP-only, P2P is WebTorrent/WebRTC only).
//
// NEW: MP4 URL → real WebTorrent seed pipeline (cmdUrlSeed).
//      When a direct MP4 URL is given and file is ≤500MB:
//        1. Fetch file as ArrayBuffer via HTTP
//        2. Wrap as File/Blob for WebTorrent.seed()
//        3. Generate a real content-derived infohash
//        4. Announce to WSS trackers
//        5. Relay real infohash+webseed to room peers
//        6. Peers join swarm and pull pieces via WebRTC
//      Files >500MB: fall back to webseed-only mode (existing).
//      Progress shown in overlay during fetch phase.
//      !hs seed <url> — explicit seed command.
//      Search bar: https:// URLs ≤500MB auto-attempt seed mode.
//
// NOTE on P2P scope: WebTorrent browser peers only connect to
//      other WebRTC-capable peers (other browsers running this
//      script). They do NOT seed to global BitTorrent clients.
//      Seeding is room-scoped via the relay infohash exchange.
//
// CARRY: All fixes from v0.00.08 and earlier.
// ============================================================
(function(){
'use strict';
...
function doFetchAndSeed(url,name,ctrl){
  fetch(url,{signal:ctrl.signal})
    ...
    .then(function(buf){
      ...
      chat('✅ Download complete — creating torrent for '+name);
      ...
      S.wt.seed(file,{announce:CFG.trackers,name:name},function(torrent){
        log('seeded! infohash:',torrent.infoHash,'peers:',torrent.numPeers);
        S.torrent=torrent;
        chat('🌱 Seeding "'+name+'" — infohash: '+torrent.infoHash);
        chat('📡 Relaying to room peers…');
        ...
        torrent.on('upload',function(bytes){
          log('uploaded',bytes,'bytes to peer');
        });
    ...
    .catch(function(e){
      ...
      chat('⚠ Fetch failed: '+e.message+' — using webseed mode');
      transition('IDLE');
...
function cmdHelp(){chat('HiveStream v'+V+': top | search <q> | pick <n> | url <https://…> | seed <https://…> | hls <m3u8-url> | magnet <uri> | pt <url> | <hash> | peers | add <…> | queue | next | clear | trackers | sync | seek <s> | stop | status');}
function cmdStatus(){
