...
const S={
  ...
  corsBlocked:new Set(),
  metadataTimeoutCount:0,
  bootTime:Date.now(), // used to ignore stale _i/_s relay replays from chat history
  metadataTimer:null,
...
function reSeedFromLibrary(it){
  if(!isCoordinator()){chat('Only coordinator can seed.');return;}
  if(S.state!=='IDLE'){chat('Stop first: !hs stop');return;}

  // Local upload items have no URL — can't re-fetch, must re-upload
  if(!it.url){
    chat('📁 "'+it.name+'" was a local upload — click 📁 upload to re-seed from device.');
    setOverlay('Local upload — re-upload to seed\nClick 📁 upload button');
    return;
  }

  // CORS-blocked? Don't bother fetching — go straight to webseed
  if(S.corsBlocked.has(it.url)){
    log('reSeedFromLibrary: URL CORS-blocked, skipping fetch');
    chat('♻ Re-seeding "'+it.name+'" (webseed proxy — server was CORS-blocked)');
  ...
  fetch(it.url,{signal:ctrl.signal})
    ...
    .then(function(buf){
      ...
      const file=new File([buf],it.name,{type:mime});
      if(!S.wt)S.wt=mkClient();
      S.wt.seed(file,{announce:CFG.trackers,name:it.name},function(torrent){
        ...
        // For large files: don't use blob URL (RAM pressure) — use renderTo
        // For small files (<100MB): blob URL is fine for immediate playback
        if(buf.byteLength<100*1024*1024){
          ...
          log('large file — using renderTo instead of blob URL');
          attachVideo(torrent,it.url);
        }
        startHUD(torrent);startSyncTimer();
        chat('▶ Re-seeding "'+it.name+'" hash:'+torrent.infoHash+' ('+Math.round(buf.byteLength/1e6)+'MB)');
      });
    })
    .catch(function(e){
      S.seedAbort=null;
      errlog('re-seed fetch failed:',e.message);
      S.corsBlocked.add(it.url);
      chat('⚠ Re-seed fetch failed — using webseed proxy for "'+it.name+'"');
      transition('IDLE');
      startSwarmThenRelay(it.syntheticHash||hashFromUrl(it.url),it.url,it.name);
    });
}
...
function cmdUpload(){
  ...
  input.onchange=function(){
    ...
    chat('📁 Uploading from device: '+file.name+' ('+sizeMB+'MB)');

    // Codec check — Android WebM screen recordings often use non-standard element IDs
    // that Chrome's MSE rejects. Warn early so user knows what to expect.
    const cc=checkCodec(file.name);
    if(!cc.playable){
      chat('⚠ Format warning: '+cc.reason);
      chat('  Seeding anyway — peers can still download. For playback, convert to MP4/H.264.');
    } else if(ext==='webm'){
      chat('ℹ WebM detected — Android screen recordings may not play in Chrome MSE.');
      chat('  Will try renderTo, then blob fallback. MP4 is more reliable.');
    }
    ...
    S.wt.seed(file,{announce:CFG.trackers,name:file.name},function(torrent){
      ...
      chat('🌱 Seeding "'+file.name+'" hash:'+torrent.infoHash);
      transition('BUFFERING');
      setOverlay('');
      S.activeWs=null;S.playbackSource='P2P-local';

      // Large files (>100MB): use renderTo — avoids holding full ArrayBuffer in RAM
      // Small files: blob URL is fine and plays immediately
      if(file.size>100*1024*1024){
        log('large upload ('+sizeMB+'MB) — using renderTo to avoid RAM pressure');
        S.videoAttached=false; // let attachVideo handle it
        attachVideo(torrent,null);
      } else {
        const blobUrl=URL.createObjectURL(file);
        wireVideoEvents(blobUrl,[]);
        S.videoAttached=true;
      ...
      // Save to library (url='' marks as local-only upload)
      ...
      loadLibraryUI();
      // Capture thumbnail 3s after seeding starts
      setTimeout(function(){
        captureThumbnail(S.video,function(thumb){
      ...
      torrent.on('wire',function(w){
        log('peer joined:',w.remoteAddress||'webrtc','total:',torrent.numPeers);
        chat('👥 '+torrent.numPeers+' peer(s) in swarm');
      });
    });
  };
  input.click();
...
function hookChat(){
  ...
  socket.on('chatMsg',function(d){
    ...
    // CyTube includes d.time (Unix seconds) on chat messages.
    // Filter out messages older than our boot time — these are history replays
    // on reconnect and should not trigger _i swarm joins or _s seeks.
    // Allow a 10s grace window for messages sent just before we booted.
    if(d.time&&(d.time*1000)<(S.bootTime-10000)){
      log('ignoring stale chat relay (pre-boot): '+raw.slice(0,60));
      return;
    }
