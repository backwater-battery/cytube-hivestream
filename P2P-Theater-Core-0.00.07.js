...
// CHANGELOG v0.00.07
// ─────────────────────────────────────────────────────────────
// FIX: Tracker list reordered based on real Termux TCP tests
//      on Samsung S23FE mobile (carrier network).
//
//      Confirmed TCP-reachable (moved to top — tried first):
//        tracker.openwebtorrent.com  ✓ WSS confirmed
//        tracker.novage.com.ua       ✓ WSS confirmed
//        tracker.opentrackr.org:1337 ✓ TCP ok
//        tracker.btorrent.xyz        ✓ TCP ok
//        framatube.org:443           ✓ TCP ok
//
//      Confirmed TCP-FAIL on mobile (moved to bottom):
//        tracker.webtorrent.dev      ✗ unreachable
//        tracker.webtorrent.io       ✗ browser fails
//        tracker.fastcast.nz         ✗ unreachable
//        peertube.cpy.re:443         ✗ unreachable
//
//      Effect: WebTorrent now attempts working trackers first,
//      finds peers faster, reduces log noise from dead trackers.
//
// NOTE: Empty HTTP response from curl WSS test means server
//       drops connection without response when hit at wrong path.
//       This is normal tracker behaviour — they only accept
//       WebSocket upgrades at /announce or /tracker/socket.
//       Actual browser WSS works via WebTorrent's internal path.
//
// CARRY: All fixes from v0.00.06 and earlier.
// ============================================================
(function(){
'use strict';
...
const CFG={
  ...
  chatDelay:100,
  trackers:[
    // ── Confirmed TCP-reachable from mobile (Termux test 2025) ──
    'wss://tracker.openwebtorrent.com',       // ✓ TCP ok, WSS confirmed working
    'wss://tracker.novage.com.ua',            // ✓ TCP ok, WSS confirmed working
    'wss://tracker.opentrackr.org:1337/announce', // ✓ TCP ok
    'wss://tracker.btorrent.xyz',             // ✓ TCP ok (browser TLS may vary)
    'wss://framatube.org:443/tracker/socket', // ✓ TCP ok
    // ── Unknown mobile reachability — kept for desktop users ──
    ...
    // ── Confirmed TCP-FAIL on mobile — kept for desktop, skipped by mobile ──
    'wss://tracker.webtorrent.dev',           // ✗ TCP fail mobile
    'wss://tracker.webtorrent.io',            // ✗ browser fails
    'wss://tracker.fastcast.nz',              // ✗ TCP fail mobile
    'wss://peertube.cpy.re:443/tracker/socket', // ✗ TCP fail mobile
  ],
...
const S={
  ...
  metadataTimer:null,
  swarmHasLeechers:false, // true when trackers report Unknown infoHash (leechers but no seeders)
  trackerConnectOk:0,     // count of trackers that connected successfully
};
