...
// CHANGELOG v0.00.03
// ─────────────────────────────────────────────────────────────
// FIX: UI injection — was targeting '#main' which doesn't exist
//      in CyTube's DOM. Now injects into '#videowrap' (above the
//      native player column) with correct Bootstrap grid classes.
//      Added multi-anchor fallback chain:
//        1. #videowrap        (CyTube standard)
//        2. #leftpane         (some CyTube layouts)
//        3. #mainpage         (outer container fallback)
//        4. document.body     (last resort)
//      Debug panel ('HS' button) now always visible regardless
//      of where root lands.
//
// FIX: Identity detection — S.myName was sometimes empty at
//      command time because the socket 'login' event fires after
//      boot. Added DOM scrape fallback: reads from
//      #username-display, #guestname-display, and the welcome
//      span used by older CyTube builds. isCoordinator() now
//      returns true immediately if rank >= 3 (owner) without
//      requiring myName to be non-empty.
//
// FIX: coordinator guard on hash/magnet commands now has a
//      rank-only fast-path so rank-5 owner can always start
//      playback even if name detection fails.
//
// NEW: Direct HTTP URL support — paste any direct .mp4/.webm
//      URL (BitChute CDN, Archive.org, etc.) into the search
//      bar or use '!hs url <url>'. Script plays it directly
//      AND adds it as a WebTorrent webseed so peers can pull
//      pieces via WebRTC. CORS caveat noted in HUD.
//
// NEW: '!hs url <direct-mp4-url>' command
//      Works for any URL ending in a video extension or
//      containing a video MIME path. No hash required —
//      a synthetic infohash is derived from the URL string
//      so the swarm is addressable and peers can join.
//
// FIX: stopAll() now clears S.videoAttached before destroying
//      torrent so a subsequent play call doesn't hit the
//      'already attached' guard.
//
// FIX: Version guard uses filename-based version (0.00.03)
//      so old instances of the script properly unload.
// ============================================================
...
const V = '0.00.03';
// Clear any older instance
if (window.__HS && window.__HS !== V) {
  log && log('old instance detected, unloading');
}
...
const KNOWN = {
  ...
  '08ada5a7a6183aae1e09d831df6748d566095a10': {
    ...
    wsFallbacks: ['https://archive.org/download/Sintel_blender/sintel-1024-surround.mp4'],
  },
  'a88fda5954e89178c372716a6a78b8180ed4dad3': {
    ...
    wsFallbacks: ['https://archive.org/download/tears-of-steel/tears_of_steel_720p.mp4'],
  },
...
function transition(newState) {
  ...
  if (!el) return;
  const L = { IDLE:'○ idle', FETCHING:'⟳ fetching', METADATA:'⟳ metadata',
               BUFFERING:'▼ buffering', PLAYING:'▶ playing', SEEDING:'▲ seeding' };
  el.textContent = L[newState] || newState;
  el.className = 'hs-state hs-state-' + newState.toLowerCase();
}
...
function encodeRelay(hash, wsUrl) {
  return '_i ' + hash + ' ' + btoa(wsUrl).replace(/=/g, '');
}
function decodeRelay(hashArg, b64Arg) {
  ...
  try {
    ...
    ws = atob(pad ? b + '='.repeat(4 - pad) : b);
  } catch(e) { warn('b64 decode:', e.message); }
  return { hash, ws };
}

// ── FIX: decodeBody — extracts full magnet href from CyTube <a> tags ──
function decodeBody(body) {
  if (!body) return body;
  let s = body;
  if (s.indexOf('<') >= 0) {
    // Pull full magnet URI from href — CyTube truncates inner text but href is always complete
...
// Derive a stable 40-char hex "infohash" from a URL string.
// This lets us treat a direct HTTP video URL as an addressable torrent swarm.
// Two users with the same URL will derive the same hash and find each other.
// Uses a simple djb2-style hash expanded to 40 hex chars.
function hashFromUrl(url) {
  ...
  // Expand to 40 hex chars by mixing with position-shifted values
  let hex = '';
  for (let i = 0; i < 5; i++) {
    const v1 = (h1 ^ (h1 >>> (i * 3 + 1))) >>> 0;
    const v2 = (h2 ^ (h2 << (i * 2 + 1))) >>> 0;
    hex += ('00000000' + v1.toString(16)).slice(-4);
    hex += ('00000000' + v2.toString(16)).slice(-4);
  }
  return hex.slice(0, 40);
}

// ── FIX: isCoordinator — rank-fast-path + better name detection ──
function isCoordinator() {
  // CyTube leader flag (most authoritative)
  if (window.CLIENT && CLIENT.leader === true) return true;
  // Rank 3+ (owner/admin) is always coordinator regardless of name
  if (S.myRank >= 3) return true;
  if (S.myRank < 1.5) return false;
  // Rank 2 (mod): check if highest-rank user in room
...
function startSwarmThenRelay(hash, ws, dn) {
  if (S.state !== 'IDLE') { log('startSwarmThenRelay: not IDLE'); return; }
  if (!S.wt) S.wt = mkClient();
  S.activeWs = ws || null;

  if (ws) {
    // HTTP-first path: play immediately, seed in background
    transition('BUFFERING');
    relay(encodeRelay(hash, ws));
    attachVideo(null, ws);
    startSyncTimer();
    const titleEl = document.getElementById('currenttitle');
    if (titleEl) titleEl.textContent = dn || hash.slice(0, 8);
    // Background P2P seeding
    idbGetTorrent(hash, function(cachedBuf) {
      let t;
      try {
        t = cachedBuf
          ? S.wt.add(new Uint8Array(cachedBuf), { announce: CFG.trackers })
          : S.wt.add(buildMagnet(hash, ws, dn));
      } catch(e) {
        t = S.wt.get(hash);
        if (!t) { warn('bg wt.add failed:', e.message); return; }
      }
      S.torrent = t;
      t.on('metadata', function() {
        log('bg metadata:', t.name);
        if (t.pieces && t.pieces.length > 0) {
          try { t.critical(0, Math.max(0, Math.floor(t.pieces.length * 0.05) - 1)); } catch(e) {}
        }
        startHUD(t);
      });
      t.on('done', function() {
      ...
      t.on('error', function(e) {
        if ((e.message || String(e)).indexOf('duplicate') >= 0) return;
        warn('bg torrent:', e.message || e);
      });
    });
    return;
  }

  // No webseed — must wait for metadata
  transition('METADATA');
  idbGetTorrent(hash, function(cachedBuf) {
    let t;
    try {
      t = cachedBuf
        ? S.wt.add(new Uint8Array(cachedBuf), { announce: CFG.trackers })
        : S.wt.add(buildMagnet(hash, null, dn));
    } catch(e) {
      t = S.wt.get(hash);
      if (!t) { errlog('swarm add failed:', e.message); transition('IDLE'); return; }
    }
    S.torrent = t;
    relay('_i ' + hash + ' -');
    bindTorrent(t);
...
function startSwarm(hash, ws, dn) {
  if (S.state !== 'IDLE') { log('startSwarm: not IDLE'); return; }
  if (!S.wt) S.wt = mkClient();
  transition('METADATA');
  S.activeWs = ws || null;
  idbGetTorrent(hash, function(cachedBuf) {
    ...
    try {
      ...
      if (!t) {
        S.wt.destroy(function() {
          S.wt = mkClient();
          S.torrent = S.wt.add(buildMagnet(hash, ws, dn));
          bindTorrent(S.torrent);
        });
        return;
      }
    }
...
function bindTorrent(t) {
  t.on('infoHash', function() { log('infoHash:', t.infoHash); });
  t.on('metadata', function() {
    ...
    if (msg.indexOf('duplicate') >= 0 && t.infoHash) {
      ...
      if (ex && ex !== t) {
        ...
        if (ex.files && ex.files.length) {
          transition('BUFFERING');
          attachVideo(ex, S.activeWs); startHUD(ex); startSyncTimer();
        } else { bindTorrent(ex); }
        return;
      }
    }
...
function attachVideo(t, wsUrl) {
  ...
  if (best.length > BLOB_LIMIT) {
    log('file >200MB, HTTP direct');
    if (wsUrl) httpFallback(wsUrl);
    else { setOverlay('File too large for P2P. No webseed.'); transition('IDLE'); }
    return;
  }
  ...
  S.video.onplaying = function() {
    S.playbackSource = 'P2P'; setOverlay(''); hideTapOverlay();
  ...
  S.video.onerror = function() {
    ...
    if (wsUrl) httpFallback(wsUrl);
    else { setOverlay('Playback failed.'); transition('IDLE'); }
  };
  ...
  best.renderTo(S.video, function(err) {
    ...
    S.renderToTimer = setTimeout(function() {
      ...
      if (S.video.readyState <= 1 && S.video.paused) {
        warn('renderTo stalled — HTTP fallback');
        errorFired = true;
        S.video.onerror = null; S.video.onplaying = null; S.video.oncanplay = null;
        if (wsUrl) httpFallback(wsUrl);
        else { setOverlay('Cannot play format.'); transition('IDLE'); }
      }
    }, 8000);
  });
}
...
function httpFallback(wsUrl) {
  ...
  if (S.torrent && S.torrent.infoHash) {
    const k = KNOWN[S.torrent.infoHash];
...
function playDirect(url, fallbacks) {
  ...
  if (p && typeof p.then === 'function') {
    p.then(function() { log('direct play started'); })
     .catch(function(e) {
       if (S.errorHandled) return;
       showTapOverlay();
     });
  }
}
...
function wireVideoEvents(currentUrl, fallbacks) {
  ...
  var cpd = null;
  S.video.oncanplay = function() {
    if (cpd) clearTimeout(cpd);
    cpd = setTimeout(function() {
      cpd = null;
      if (S.video && S.video.paused && S.video.readyState >= 3) showTapOverlay();
    }, 800);
  };
  S.video.onended = function() { playlistAdvance(); };
  S.video.onerror = function() {
    ...
    if (fallbacks && fallbacks.length > 0) {
      ...
      warn('trying fallback:', next.slice(0, 80));
      S.video.removeAttribute('src'); S.video.load();
      S.errorHandled = false;
      wireVideoEvents(next, rest);
      S.video.src = next; S.video.load();
...
function showTapOverlay() {
  ...
  tap.onclick = function() {
    S.video.muted = true;
    S.video.play().catch(function(e) { errlog('tap play:', e.message); });
  };
  const vw = document.getElementById('hs-vwrap');
  if (vw) vw.appendChild(tap);
}
...
function startHUD(t) {
  ...
  S.hudTimer = setInterval(function() {
    ...
    if (el) el.textContent = '[' + (S.playbackSource || 'P2P') + '] ' + pct + '% · ' + _t.numPeers + 'p · ' + spd;
  }, 2000);
}
...
function stopAll(announce) {
  ...
  // FIX: clear videoAttached BEFORE nulling video state so next play works
  S.videoAttached = false;
  S.activeWs = null; S.errorHandled = false; S.playbackSource = 'P2P';
  if (S.renderToTimer) { clearTimeout(S.renderToTimer); S.renderToTimer = null; }
  if (S.video) {
    S.video.onplaying = null; S.video.oncanplay = null;
    S.video.onerror = null; S.video.onended = null;
    S.video.pause();
    S.video.removeAttribute('src');
    S.video.load();
  }
  transition('IDLE');
  setOverlay('Nothing Playing — search, paste URL, or use !hs help');
...
function applySync(t) {
  ...
  if (Math.abs(S.video.currentTime - t) > CFG.syncThres) {
    log('sync seek:', t); S.video.currentTime = t;
  }
}

// ── NEW: Direct URL command ───────────────────────────────────
// Plays any direct video URL (MP4/WebM/etc) immediately.
// Simultaneously adds as WebTorrent webseed so peers can pull pieces via WebRTC.
// A synthetic 40-char infohash is derived from the URL so the swarm is addressable.
function cmdUrl(url) {
  if (!isCoordinator()) { chat('Only the coordinator can start videos.'); return; }
  if (S.state !== 'IDLE') { chat('Already ' + S.state + '. Use !hs stop first.'); return; }

  url = url.trim();
  if (!url.startsWith('http')) { chat('Need a full https:// URL'); return; }

  const hash = hashFromUrl(url);
  // Derive display name from URL path
  const parts = url.split('/');
  const filename = parts[parts.length - 1].split('?')[0] || 'video';
  const name = decodeURIComponent(filename);

  chat('Loading URL: ' + name);
  log('cmdUrl hash=' + hash + ' url=' + url.slice(0, 80));

  // Start swarm with URL as webseed — this relays to room and plays
...
function playlistAdvance() {
  ...
  if (next >= S.playlist.length) {
    log('playlist exhausted'); S.playlistPos = -1;
    chat('Playlist complete.'); renderPlaylistUI(); return;
  }
  S.playlistPos = next;
  renderPlaylistUI();
  ...
  if (S.state !== 'IDLE') {
    stopAll(false);
    setTimeout(function() { playlistPlay(item); }, 500);
  } else { playlistPlay(item); }
}
function playlistPlay(item) {
  if (!item) return;
  chat('▶ Queue: ' + item.name);
...
function playlistClear() {
  S.playlist = []; S.playlistPos = -1;
  renderPlaylistUI(); chat('Playlist cleared.');
}
function playlistShow() {
  if (!S.playlist.length) { chat('Playlist empty.'); return; }
  S.playlist.forEach(function(it, i) {
    const m = i === S.playlistPos ? '▶ ' : i < S.playlistPos ? '✓ ' : '  ';
    chat(m + (i+1) + '. ' + it.name);
  });
}
function renderPlaylistUI() {
  const list = document.getElementById('hs-playlist-items');
  if (!list) return;
  list.innerHTML = '';
  const titleEl = document.getElementById('hs-pl-title');
  if (titleEl) titleEl.textContent = (document.getElementById('hs-playlist-body').classList.contains('open') ? '▾' : '▸') + ' queue (' + S.playlist.length + ')';
  if (!S.playlist.length) {
    list.innerHTML = '<div style="color:#333;font:10px Courier New,monospace;padding:4px 6px">queue empty — paste magnet or URL below</div>';
    return;
  }
  S.playlist.forEach(function(item, i) {
    ...
    div.querySelector('button').onclick = function(e) {
      e.stopPropagation();
      S.playlist.splice(parseInt(this.getAttribute('data-i')), 1);
      if (S.playlistPos > parseInt(this.getAttribute('data-i'))) S.playlistPos--;
      renderPlaylistUI();
    };
...
function cmdAdd(raw) {
  if (!raw) { chat('Usage: !hs add <magnet|hash|url|peertube-url>'); return; }
  raw = raw.trim();
  if (raw.startsWith('magnet:')) {
    const hm = raw.match(/urn:btih:([0-9a-f]{40})/i);
    const dm = raw.match(/[?&]dn=([^&]+)/);
    const hash = hm ? hm[1].toLowerCase() : null;
    const dn = dm ? decodeURIComponent(dm[1].replace(/\+/g,' ')) : null;
    if (!hash) { chat('Could not parse magnet hash'); return; }
    playlistAdd({ type: 'magnet', value: raw, name: dn || hash.slice(0,8) + '…' });
    return;
  }
  if (/^[0-9a-f]{40}$/i.test(raw)) {
    const h = raw.toLowerCase();
    playlistAdd({ type: 'hash', value: h, name: (KNOWN[h] ? KNOWN[h].name : h.slice(0,8) + '…') });
    return;
  }
  if (raw.startsWith('http')) {
    // Check if it looks like a PeerTube URL
    try {
      const u = new URL(raw);
      const m = u.pathname.match(/\/(?:videos\/watch|w)\/([^/?#]+)/);
      if (m) { playlistAdd({ type: 'pt', value: raw, name: u.hostname + '/' + m[1] }); return; }
    } catch(e) {}
    // Direct media URL
    const parts = raw.split('/');
    const name = decodeURIComponent(parts[parts.length-1].split('?')[0]) || 'video';
    playlistAdd({ type: 'url', value: raw, name: name });
    return;
  }
  chat('Unrecognised format. Try a magnet:, https://, or 40-char hash.');
}

// ── TRACKER STATUS ───────────────────────────────────────────
function cmdTrackers() {
  chat('Pinging ' + CFG.trackers.length + ' trackers…');
  const panel = document.getElementById('hs-tracker-panel');
  ...
  CFG.trackers.forEach(function(url) {
    ...
    updateTrackerUI(safeKey, '…', '#555');
    let ws;
    const timer = setTimeout(function() {
      updateTrackerUI(safeKey, 'timeout', '#ff9939');
      if (++done === CFG.trackers.length) chat('Trackers: ' + ok.n + '/' + done + ' ok');
    }, 5000);
    try {
      ws = new WebSocket(url);
      ws.onopen = function() { clearTimeout(timer); ok.n++; updateTrackerUI(safeKey, '✓ ok', '#39ff14'); ws.close(); if (++done === CFG.trackers.length) chat('Trackers: ' + ok.n + '/' + done + ' ok'); };
      ws.onerror = function() { clearTimeout(timer); updateTrackerUI(safeKey, 'error', '#ff3939'); if (++done === CFG.trackers.length) chat('Trackers: ' + ok.n + '/' + done + ' ok'); };
    } catch(e) {
      clearTimeout(timer); updateTrackerUI(safeKey, 'fail', '#ff3939');
      if (++done === CFG.trackers.length) chat('Trackers: ' + ok.n + '/' + done + ' ok');
    }
  });
}
function updateTrackerUI(safeKey, status, color) {
  const el = document.getElementById('hs-tr-' + safeKey);
  if (el) { el.textContent = status; el.style.color = color; }
}

// ── SEARCH & PICK ────────────────────────────────────────────
function cmdArchive(q) {
  if (!isCoordinator()) { chat('Only coordinator can search.'); return; }
  if (S.state !== 'IDLE') { chat('Stop first.'); return; }
  ...
  fetch(CFG.archiveApi + '?q=' + encodeURIComponent(q + ' AND mediatype:movies') + '&fl[]=identifier,title&sort[]=downloads+desc&rows=20&output=json')
    ...
    .then(function(d) {
      ...
      showGrid(S.results);
      S.results.slice(0,8).forEach(function(v,i) { chat((i+1)+'. '+v.name); });
    })
...
function cmdTop() {
  ...
  fetch(CFG.topApi)
    ...
    .then(function(d){
      ...
      showGrid(S.results);
      S.results.slice(0,8).forEach(function(v,i){ chat((i+1)+'. ['+fmtDur(v.duration)+'] '+v.name); });
    })
...
function cmdSearch(q) {
  ...
  fetch(CFG.searchApi + '?search=' + encodeURIComponent(q) + '&count=40&sort=-views&nsfw=false')
    ...
    .then(function(d){
      ...
      showGrid(S.results);
      S.results.slice(0,8).forEach(function(v,i){ chat((i+1)+'. ['+fmtDur(v.duration)+'] '+v.name+' ('+(v.account&&v.account.host||'?')+')'); });
    })
...
function cmdPick(n) {
  ...
  fetch(host+'/api/v1/videos/'+uuid)
    ...
    .then(function(data){
      ...
      if (!files.length) { transition('IDLE'); setOverlay('Nothing Playing'); chat('⚠ "'+data.name+'" HLS-only. Try another.'); return; }
      ...
      chat('Playing: "'+data.name+'" — relaying');
...
function cmdMagnet(magnet) {
  ...
  if (!magnet.startsWith('magnet:')) {
    const idx = magnet.indexOf('magnet:');
    if (idx >= 0) magnet = magnet.slice(idx);
  }
...
function cmdPt(url) {
  ...
  try {
    ...
    const m = u.pathname.match(/\/(?:videos\/watch|w)\/([^/?#]+)/);
    if (!m) throw new Error('Bad PeerTube URL');
    vid = m[1];
  ...
  fetch(origin+'/api/v1/videos/'+vid)
    ...
    .then(function(data){
      ...
      chat('Playing: "'+data.name+'"'); setOverlay('Connecting…');
      transition('IDLE'); startSwarmThenRelay(hash, ws, data.name);
    })
...
function cmdPickHash(hash) {
  ...
  chat('Loading: '+name);
  if (k && k.torrentUrl) {
    transition('METADATA'); setOverlay('Loading…');
    if (!S.wt) S.wt = mkClient();
    ...
    try { t = S.wt.add(k.torrentUrl, {announce:CFG.trackers}); }
      ...
      try { t = S.wt.add(buildMagnet(hash,ws,name)); }
      catch(e2) { t = S.wt.get(hash); if(!t){ errlog('all add failed'); transition('IDLE'); return; } }
    }
...
// ── FIX: UI INJECTION — correct CyTube DOM anchors ───────────
// CyTube layout: #videowrap contains the player column.
// We inject #hs-root above #videowrap, inside the same Bootstrap column.
// Fallback chain tries multiple known CyTube container IDs.
function buildUI() {
  ...
  #hs-overlay{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#282828;font:12px 'Courier New',monospace;pointer-events:none;z-index:4;text-align:center;padding:8px;}
  ...
  root.innerHTML = `
  <div id="hs-toolbar">
    <input id="hs-sinput" placeholder="search, paste magnet or https://…">
  ...
  <div id="hs-vwrap">
    <div id="hs-hud"></div>
    <div id="hs-overlay">Nothing Playing — search, paste URL, or !hs help</div>
  </div>
  <div id="hs-controls">
    <button id="hs-playbtn">▶</button>
  ...
  <div id="hs-tracker-panel"><h4>WSS Tracker Status</h4></div>
  <div id="hs-playlist">
    <div id="hs-playlist-header">
      <span id="hs-pl-title">▸ queue (0)</span>
      <button class="hs-btn" id="hs-pl-next" style="font-size:9px">next ▶</button>
      <button class="hs-btn danger" id="hs-pl-clear" style="font-size:9px">clear</button>
    </div>
    <div id="hs-playlist-body">
      ...
      <div id="hs-pl-input-row">
        <input id="hs-pl-input" placeholder="magnet: or https://…">
        <button class="hs-btn" id="hs-pl-addbtn">+ add</button>
      </div>
    </div>
  </div>
  `;

  // Create video element separately (innerHTML can't create <video> with events cleanly)
  const vid = document.createElement('video');
  vid.id = 'hs-video'; vid.autoplay = true; vid.playsInline = true; vid.controls = false;
  S.video = vid;
  const vwrap = root.querySelector('#hs-vwrap');
  vwrap.insertBefore(vid, vwrap.firstChild);

  // Build tracker rows
  const trkPanel = root.querySelector('#hs-tracker-panel');
  CFG.trackers.forEach(function(url) {
    const key = url.replace('wss://','').replace('/announce','').replace('/tracker/socket','');
    const safeKey = key.replace(/[^a-z0-9]/gi,'_');
    const row = document.createElement('div');
    row.className = 'hs-tr-row';
    row.innerHTML = '<span class="hs-tr-name">' + escHtml(key) + '</span><span id="hs-tr-' + safeKey + '" style="color:#333">—</span>';
    trkPanel.appendChild(row);
  });

  // ── FIX: injection anchor — try CyTube DOM IDs in priority order ──
  // #videowrap = the main video column div in CyTube
  // #leftpane  = left column in some CyTube layouts
  // #main-col  = used in some forks
  // body.prepend = last resort (still visible, just at top of page)
  ...
  for (let i = 0; i < anchors.length; i++) {
    ...
    if (el) {
      ...
      log('UI injected into', anchors[i]);
      injected = true;
      break;
    }
  }
  if (!injected) {
    ...
    log('UI injected into body (fallback)');
  }
  ...
  document.getElementById('hs-sinput').onkeydown = function(e) {
    ...
    document.getElementById('hs-sbtn').onclick();
  };
  ...
  document.getElementById('hs-playbtn').onclick = function() {
    if (!S.video) return;
    S.video.paused ? S.video.play().catch(function(){}) : S.video.pause();
  };
  document.getElementById('hs-seek').oninput = function() {
    if (S.video && S.video.duration) S.video.currentTime = (this.value/1000)*S.video.duration;
  };
  document.getElementById('hs-mutebtn').onclick = function() {
    if (!S.video) return; S.video.muted=!S.video.muted; this.textContent=S.video.muted?'🔇':'🔊';
  };
  ...
  document.getElementById('hs-playlist-header').onclick = function(e) {
    ...
    document.getElementById('hs-playlist-body').classList.toggle('open');
    renderPlaylistUI();
  };
  ...
  document.getElementById('hs-pl-addbtn').onclick = function() {
    const inp=document.getElementById('hs-pl-input'), v=inp.value.trim();
    if (v) { cmdAdd(v); inp.value=''; }
  };
  document.getElementById('hs-pl-input').onkeydown = function(e) {
    if (e.key==='Enter') { const v=this.value.trim(); if(v){cmdAdd(v);this.value='';} }
  };
...
function buildDebugPanel() {
  ...
  toggle.id = 'hs-dbtoggle'; toggle.textContent = 'HS debug';
  ...
  document.getElementById('hs-dbclear').onclick = function() {
    dbLines.length=0; const el=document.getElementById('hs-dblog'); if(el) el.textContent='';
  };
...
function dumpStatus() {
  ...
  log('torrent:',S.torrent?S.torrent.name+' peers='+S.torrent.numPeers:'null');
...
function showGrid(items) {
  ...
  items.forEach(function(r,i) {
    ...
    const warn=host&&!r._archive&&!WEBTORRENT_INSTANCES.includes(host)?' ⚠':'';
    dur.textContent=fmtDur(r.duration)+(host?' · '+host:'')+warn;
...
function setOverlay(txt) {
  const el=document.getElementById('hs-overlay'); if(!el) return;
  el.style.display=txt?'flex':'none'; el.textContent=txt;
}
...
function idbStoreTorrent(t) {
  ...
  function tryStore(n) {
    ...
    } catch(e){ warn('IDB store error:',e.message); }
  }
  tryStore(6);
}
function idbGetTorrent(hash,cb) {
  ...
  try {
    const req=S.idb.transaction('torrents','readonly').objectStore('torrents').get(hash);
    req.onsuccess=function(){cb(req.result||null);}; req.onerror=function(){cb(null);};
  } catch(e){cb(null);}
}
...
function hookChat() {
  ...
  socket.on('chatMsg', function(d) {
    ...
    // Special case: entire body is a magnet URI (CyTube auto-linked)
    if (body.startsWith('magnet:')) { if(d.username===S.myName) cmdMagnet(body); return; }
    // Special case: 'magnet <full-uri>'
    const magnetKeyword = body.match(/^magnet\s+(magnet:.+)$/i);
    if (magnetKeyword) { if(d.username===S.myName) cmdMagnet(magnetKeyword[1]); return; }
  ...
  socket.on('setLeader', function(name){ if(name===S.myName) { log('setLeader: we are leader'); if(S.video&&S.torrent) startSyncTimer(); } });
  socket.on('changeMedia', function(data){
    if (S.state!=='IDLE'&&data&&data.type&&data.type!=='cu') { log('changeMedia: stopping HS'); stopAll(false); }
  });
  log('chat hooked');
}
...
function onCmd(sender, cmd, args, body) {
  ...
  if (cmd==='_i') {
    ...
    log('_i hash='+hash+' ws='+ws.slice(0,60));
    ...
    if (cached&&cached.files&&cached.files.length) {
      S.torrent=cached; S.activeWs=ws||null; transition('BUFFERING');
      attachVideo(cached,ws||null); startHUD(cached); startSyncTimer(); return;
    }
    startSwarm(hash,ws||null,null); return;
  }
  ...
  if (cmd==='url'&&sender===S.myName)      { cmdUrl(args[0]||''); return; }
...
function cmdHelp() {
  chat('HiveStream v'+V+': top | search <q> | pick <n> | url <https://…> | magnet <uri> | pt <url> | <hash> | add <magnet|url|hash> | queue | next | clear | trackers | sync | seek <s> | stop | status');
}
function cmdStatus() {
  ...
  chat(S.state+' · '+pct+'% · '+t.numPeers+'p · '+fmtSpd(t.downloadSpeed)+'▼ '+fmtSpd(t.uploadSpeed)+'▲ | queue:'+q);
}
function cmdInfo(n) {
  const v=S.results[n-1]; if(!v){chat('No result '+n);return;}
  chat('['+n+'] '+v.name+' | '+fmtDur(v.duration)+' | '+(v.account&&v.account.host||'?'));
}
...
function detectSelf() {
  try {
    if (window.CLIENT) {
      S.myName=CLIENT.name||''; S.myRank=CLIENT.rank!==undefined?CLIENT.rank:-1;
    }
  } catch(e) {}
  // DOM fallbacks for name
  if (!S.myName) {
    const selectors=['#username-display','#guestname-display','.username'];
    for (let i=0;i<selectors.length;i++) {
      const el=document.querySelector(selectors[i]);
      if (el&&el.textContent.trim()) { S.myName=el.textContent.trim(); break; }
    }
  }
  if (!S.myName) {
    const w=document.getElementById('welcome');
    if (w) S.myName=w.textContent.replace(/^Welcome,?\s*/i,'').replace(/[!.]+$/,'').trim();
  }
