...
// CHANGELOG v0.00.10
// ─────────────────────────────────────────────────────────────
// ARCH: Synthetic hash clarified — hashFromUrl() is a fallback
//       for webseed-only mode. wt.seed(file) generates a REAL
//       SHA1 infohash from file content. These are different:
//       synthetic = coordination hash (URL pointer, no real pieces)
//       real      = content hash (actual file data in WebRTC)
//
// NEW: IDB Library (schema v3, store 'library')
//      After wt.seed() completes, stores:
//        - realHash (SHA1 from file content)
//        - url (original source URL)
//        - name, size, added (timestamp)
//        - thumb (base64 canvas capture at 50% playback)
//      On boot: library items auto-reseed if URL still reachable.
//      !hs library — lists stored items in chat
//      !hs forget <hash> — removes item from library
//
// NEW: Thumbnail generation — canvas capture at 50% duration
//      stored as 160x90 dataURL in IDB library entry.
//      Shown in library UI panel as clickable cards.
//
// NEW: Library UI panel — collapsible panel below playlist.
//      Shows ingested content with thumbnails, name, size.
//      Click to re-play and re-seed to room.
//
// NEW: Upload from device — file input button lets user pick
//      a local MP4/WebM file, seeds it directly from device
//      storage (no server fetch needed). Real hash generated.
//      !hs upload — triggers file picker via chat command.
//
// FIX: Synthetic hash mode now clearly labeled in HUD as
//      [WS-proxy] to distinguish from real P2P seeding [P2P].
//      Real seed mode shows [P2P-seed] + peer count.
//
// CARRY: All fixes from v0.00.09 and earlier.
// ============================================================
(function(){
'use strict';
...
function cmdUrlSeed(url){
  ...
  const hash=hashFromUrl(url); // synthetic hash for progress display

  chat('🔍 Checking file size: '+name);
  log('cmdUrlSeed checking:',url.slice(0,80));

  // HEAD request to get content-length before committing to download
  ...
  fetch(url,{method:'HEAD',signal:ctrl.signal})
    .then(function(r){
      ...
      log('HEAD content-length:',size,'bytes');
      if(size>SEED_LIMIT||size===0){
        if(size>SEED_LIMIT){
          chat('📦 '+name+' is '+Math.round(size/1e6)+'MB — too large to buffer (>500MB). Using webseed mode.');
        } else {
          chat('📦 Size unknown — using webseed mode for '+name);
        }
        // Fall back to webseed-only (existing behaviour)
        S.seedAbort=null;
        startSwarmThenRelay(hash,url,name);
        return;
      }
      // File is small enough — fetch and seed for real
      chat('📥 Fetching '+Math.round(size/1e6)+'MB to seed — this may take a moment…');
      transition('FETCHING');
      setOverlay('Fetching '+Math.round(size/1e6)+'MB\nfor real P2P seed…');
      doFetchAndSeed(url,name,ctrl);
    })
    .catch(function(e){
      if(e.name==='AbortError'){log('seed fetch aborted');return;}
      warn('HEAD failed:',e.message,'— using webseed mode');
      S.seedAbort=null;
      startSwarmThenRelay(hash,url,name);
    });
}

function doFetchAndSeed(url,name,ctrl){
  fetch(url,{signal:ctrl.signal})
    ...
    .then(function(buf){
      S.seedAbort=null;
      log('fetch complete:',buf.byteLength,'bytes — seeding');
      setOverlay('Creating torrent from file…');
      chat('✅ Download complete ('+Math.round(buf.byteLength/1e6)+'MB) — hashing for P2P seed…');

      if(!S.wt)S.wt=mkClient();

      // Wrap ArrayBuffer as File for WebTorrent.seed()
      // This generates a REAL SHA1 infohash from file content
      const file=new File([buf],name,{type:'video/mp4'});
      const syntheticHash=hashFromUrl(url); // keep for fallback reference

      S.wt.seed(file,{announce:CFG.trackers,name:name},function(torrent){
        log('seeded! REAL infohash:',torrent.infoHash,'(synthetic was:',syntheticHash+')');
        S.torrent=torrent;
        chat('🌱 Real P2P seed: "'+name+'" — hash: '+torrent.infoHash);
        chat('📡 Relaying real hash to room peers via WebRTC…');

        // Relay REAL content-derived infohash + original URL as webseed
        relay(encodeRelay(torrent.infoHash,url));

        // Play locally via blob URL from the seeded file
        transition('BUFFERING');
        setOverlay('');
        ...
        // Capture thumbnail at 50% after a 3s delay
        setTimeout(function(){
          captureThumbnail(S.video,function(thumb){
            if(thumb){
              libEntry.thumb=thumb;
              idbLibrarySave(libEntry);
              loadLibraryUI();
              log('thumbnail captured for',name);
            }
          });
        },3000);

        torrent.on('wire',function(w){
          log('peer joined seed swarm:',w.remoteAddress||'webrtc','total:',torrent.numPeers);
          chat('👥 Peer joined: '+torrent.numPeers+' peer(s) in swarm');
        });
        torrent.on('upload',function(bytes){
          log('uploaded',Math.round(bytes/1024)+'KB to peer — total ul:',Math.round(torrent.uploaded/1024)+'KB');
        });
    ...
    .catch(function(e){
      S.seedAbort=null;
      if(e.name==='AbortError'){log('seed fetch aborted');transition('IDLE');setOverlay('Nothing Playing — search, paste URL, or !hs help');return;}
      errlog('fetch failed:',e.message);
      chat('⚠ Fetch failed ('+e.message+') — falling back to webseed proxy mode');
      chat('ℹ In webseed proxy: peers pull from source server, not from you. Real seeding requires file download.');
      transition('IDLE');
...
function bindTorrent(t){
  ...
  t.on('warning',function(w){
    ...
    // Detect "Unknown infoHash" — tracker sees the hash but has no seeders registered.
    // This means other peers are also looking but nobody has the file.
    if(msg.indexOf('Unknown infoHash')>=0){
      S.swarmHasLeechers=true;
      log('swarm has leechers but no seeders for',t.infoHash);
      return; // don't spam the log with these — one detection is enough
    }
    // Detect successful tracker connections (absence of "Error connecting")
