...
// CHANGELOG v0.00.04
// ─────────────────────────────────────────────────────────────
// FIX: b64 decode error — relay sends '-' to mean "no webseed".
//      decodeRelay() now checks for '-' or empty before calling
//      atob(), returning ws:''. Fixes the console warning on
//      every magnet relay receive.
//
// FIX: Ghost IDLE→PLAYING transition after large-file magnet.
//      When a magnet resolves to a file >200MB with no webseed,
//      the old code called transition('IDLE') then fell through
//      to dead code that called transition('PLAYING'). Fixed by
//      returning early and keeping the torrent seeding in
//      background. State stays at SEEDING once download completes.
//
// FIX: archivePick() had a stray transition('PLAYING') call that
//      ran unconditionally after async fetch. Removed — PLAYING
//      is now set by wireVideoEvents onplaying handler only.
//
// NEW: Codec/container pre-check before playback attempt.
//      Uses MediaSource.isTypeSupported() and canPlayType() to
//      detect MKV, HEVC/H.265, AV1 support before attaching.
//      On unsupported codec: shows clear overlay message,
//      keeps torrent seeding in background (peers still benefit),
//      and posts to chat: "⚠ codec not supported in this browser".
//      Supported: MP4/H.264, WebM/VP8/VP9 (all browsers).
//      MKV/H.265: only Safari/iOS. AV1: Firefox/Chrome desktop.
//
// NEW: !hs peers — shows WebRTC wire details in chat.
//      Useful for confirming P2P is actually seeding to others.
//      Output: wire type (webrtc/webSeed), remote addr, speeds.
//
// NEW: !hs wires — alias for !hs peers.
//
// FIX: Large magnet with no webseed now shows:
//      "Seeding <name> — no direct playback (no webseed URL).
//       Use !hs url <direct-url> to play, or find a webseed."
//      Rather than the confusing "File too large for P2P" msg.
//
// ARCH: Version scheme continues: 0.00.04
// ============================================================
...
// FIX: dash '-' means "no webseed" — skip atob entirely
function decodeRelay(hashArg,b64Arg) {
  const hash=(hashArg||'').toLowerCase();
  let ws='';
  const b=b64Arg||'';
  // '-' is the explicit sentinel for "no webseed URL"
...
// ── decodeBody — extracts full magnet href from CyTube <a> tags ──
...
// ── NEW: CODEC DETECTION ─────────────────────────────────────
// Returns an object: { playable: bool, reason: string, suggestedExt: string }
// Uses both MediaSource.isTypeSupported and video.canPlayType for belt-and-suspenders.
//
// Browser support reality:
//   MP4 + H.264 (AVC):  all browsers ✓
//   WebM + VP8/VP9:     all browsers ✓
//   MP4 + H.265 (HEVC): Safari/iOS only ✗ Chrome/Firefox/Brave
//   MKV container:      not supported in any browser natively
//   AV1:                Chrome/Firefox desktop ✓, mobile inconsistent
//   x265 encoded MKV:   never playable in browser — needs transcode
function checkCodec(filename) {
  const ext = (filename.split('.').pop()||'').toLowerCase();
  const v = document.createElement('video');

  // MKV is never supported natively — MediaSource can't handle it
  if (ext==='mkv' || ext==='avi' || ext==='wmv' || ext==='flv') {
    return {
      playable: false,
      reason: 'Container .'+ext+' is not supported in browsers. Need MP4 or WebM.',
      container: ext
    };
  }
  ...
  if (ext==='mp4'||ext==='m4v') {
    ...
    if (h264==='probably'||h264==='maybe') return {playable:true,reason:'MP4/H.264 ok',container:'mp4'};
    if (hevc==='probably'||hevc==='maybe') return {playable:true,reason:'MP4/HEVC ok (Safari)',container:'mp4'};
    return {playable:false,reason:'MP4 but no supported codec found',container:'mp4'};
  }
  ...
  if (ext==='webm') {
    ...
    if (vp9==='probably'||vp9==='maybe'||vp8==='probably'||vp8==='maybe')
      return {playable:true,reason:'WebM/VP9 ok',container:'webm'};
    return {playable:false,reason:'WebM but no VP8/VP9 support',container:'webm'};
  }

  // OGV, OGG
  if (ext==='ogv'||ext==='ogg') {
    const t=v.canPlayType('video/ogg; codecs="theora"');
    return {playable:t!=='',reason:'OGG/Theora '+(t||'not supported'),container:'ogg'};
  }

  // Unknown extension — try generic canPlayType guesses
  const generic = v.canPlayType('video/mp4')||v.canPlayType('video/webm');
  return {playable:generic!=='',reason:'Unknown ext .'+ext+' — assume '+( generic?'ok':'unsupported'),container:ext};
}

// Check if filename contains x265/HEVC encoding hint (common in torrent names)
function filenameHasHevc(name) {
  return /x265|h\.265|hevc|10bit/i.test(name);
}
...
function startSwarmThenRelay(hash,ws,dn) {
  ...
  if (ws) {
    ...
    idbGetTorrent(hash,function(cachedBuf){
      let t;
      try {
        t=cachedBuf?S.wt.add(new Uint8Array(cachedBuf),{announce:CFG.trackers}):S.wt.add(buildMagnet(hash,ws,dn));
      } catch(e) {
        t=S.wt.get(hash);
  ...
  // No webseed — wait for metadata, then decide
  transition('METADATA');
  idbGetTorrent(hash,function(cachedBuf){
    let t;
    try {
      t=cachedBuf?S.wt.add(new Uint8Array(cachedBuf),{announce:CFG.trackers}):S.wt.add(buildMagnet(hash,null,dn));
    } catch(e) {
      t=S.wt.get(hash);
...
// Background torrent — already playing via HTTP, just seed to peers
...
function bindTorrent(t) {
  ...
  t.on('metadata',function(){
    ...
    // ── Codec check before attempting attachment ──────────────
    ...
    if (best) {
      // Check for x265/HEVC in filename — dead giveaway it won't play
      if (filenameHasHevc(best.name||t.name||'')) {
        const msg='⚠ "'+best.name+'" is HEVC/x265 — not supported in Chrome/Firefox/Brave. Only Safari can play HEVC. Seeding continues in background.';
        chat(msg);
        setOverlay('HEVC/x265 — unsupported codec\nSeeding in background ▲');
        log('codec block: HEVC detected in', best.name);
        // Keep seeding — don't transition to PLAYING, let done→SEEDING handle it
        transition('SEEDING');
        startHUD(t);
      ...
      const codecCheck = checkCodec(best.name||'');
      if (!codecCheck.playable) {
        const msg='⚠ "'+best.name+'" — '+codecCheck.reason+'. Seeding in background.';
        chat(msg);
        setOverlay(codecCheck.reason+'\nSeeding in background ▲');
        log('codec block:',codecCheck.reason);
        transition('SEEDING');
        startHUD(t);
        t.on('done',function(){idbStoreTorrent(t);});
        t.on('wire',function(w){log('peer:',w.remoteAddress||'webrtc','total:',t.numPeers);});
    ...
    if (fileSize>BLOB_LIMIT) {
      // Large file, no webseed — can't play but keep seeding
      // FIX: was incorrectly calling transition('IDLE') then ghost PLAYING
      const msg='⚠ "'+t.name+'" is '+Math.round(fileSize/1e6)+'MB — too large for browser P2P render without a webseed URL. Add one with: !hs url <direct-mp4-url>';
      chat(msg);
      setOverlay('No webseed for large file.\nAdd: !hs url <direct-url>\nSeeding in background ▲');
      log('large file no webseed — seeding only');
      transition('SEEDING');
      startHUD(t);
      t.on('done',function(){idbStoreTorrent(t);});
      t.on('wire',function(w){log('peer:',w.remoteAddress||'webrtc','total:',t.numPeers);});
    ...
    // Small enough for P2P render — proceed
  ...
  t.on('ready',function(){
    if (S.state==='BUFFERING') { transition('PLAYING'); chat('"'+t.name+'" ready — '+t.numPeers+'p'); }
  });
  t.on('done',function(){
    transition('SEEDING');
    idbStoreTorrent(t);
...
function attachVideo(t,wsUrl) {
  if (S.videoAttached){log('attachVideo: already attached');return;}
  ...
  if (best.length>BLOB_LIMIT) {
    log('file >200MB — P2P render impossible, checking wsUrl');
    if (wsUrl) httpFallback(wsUrl);
    else {
      setOverlay('Large file, no webseed.\nAdd: !hs url <direct-url>');
      // Don't transition to IDLE — torrent is still seeding
    }
    return;
  }
...
function playDirect(url,fallbacks) {
  ...
  if(p&&typeof p.then==='function'){
    p.then(function(){log('direct play started');})
     .catch(function(e){if(!S.errorHandled) showTapOverlay();});
  }
}
...
function wireVideoEvents(currentUrl,fallbacks) {
  ...
  S.video.oncanplay=function(){
    ...
    cpd=setTimeout(function(){cpd=null;if(S.video&&S.video.paused&&S.video.readyState>=3)showTapOverlay();},800);
  };
  S.video.onended=function(){playlistAdvance();};
  S.video.onerror=function(){
...
function startHUD(t) {
  ...
  S.hudTimer=setInterval(function(){
    ...
    // Count WebRTC wires vs webseed wires separately
    let rtcPeers=0,wsPeers=0;
    if(_t.wires){_t.wires.forEach(function(w){if(w.type==='webSeed')wsPeers++;else rtcPeers++;});}
    const src=S.playbackSource||'P2P';
    const el=document.getElementById('hs-hud');
    if(el) el.textContent='['+src+'] '+pct+'% · '+rtcPeers+'🔗 '+wsPeers+'🌐 · '+spd;
  },2000);
}
...
function startSyncTimer() {
  ...
  S.syncTimer=setInterval(function(){
    if(S.video&&!S.video.paused&&isCoordinator()) relay('_s '+S.video.currentTime.toFixed(2));
  },CFG.syncMs);
}
function applySync(t) {
  if(!S.video||isNaN(t)) return;
  if(Math.abs(S.video.currentTime-t)>CFG.syncThres){log('sync seek:',t);S.video.currentTime=t;}
}

// ── NEW: PEER/WIRE INSPECTION ─────────────────────────────────
// Shows WebRTC and webseed wire details in chat.
// This is the primary way to verify P2P is actually distributing data on mobile
// (since there's no DevTools console on Brave Android).
function cmdPeers() {
  if (!S.torrent) { chat('No active torrent.'); return; }
  const t=S.torrent;
  const total=t.numPeers||(t.wires?t.wires.length:0);
  if (!total) { chat('No peers connected. (Trackers: wss only '+['tracker.openwebtorrent.com','tracker.novage.com.ua'].join(', ')+' seem reachable)'); return; }
  chat('Torrent: '+t.name);
  chat('Total peers: '+total+' | downloaded: '+Math.round(t.downloaded/1024)+'KB | uploaded: '+Math.round(t.uploaded/1024)+'KB');
  if (t.wires) {
    t.wires.forEach(function(w,i){
      const type=w.type||'unknown';
      const addr=w.remoteAddress||w.id||'?';
      const dl=fmtSpd(w.downloadSpeed?w.downloadSpeed():0);
      const ul=fmtSpd(w.uploadSpeed?w.uploadSpeed():0);
      const interested=w.peerInterested?'interested':'not interested';
      chat('['+i+'] '+type+' '+addr.slice(0,30)+' '+dl+'▼ '+ul+'▲ '+interested);
    });
  }
}
...
function playlistShow(){
  ...
  S.playlist.forEach(function(it,i){
    chat((i===S.playlistPos?'▶ ':i<S.playlistPos?'✓ ':'  ')+(i+1)+'. '+it.name);
  });
}
function renderPlaylistUI() {
  ...
  const isOpen=document.getElementById('hs-playlist-body')&&document.getElementById('hs-playlist-body').classList.contains('open');
  ...
  S.playlist.forEach(function(item,i){
    ...
    div.querySelector('button').onclick=function(e){
      ...
      if(S.playlistPos>idx) S.playlistPos--;
      renderPlaylistUI();
    };
...
function cmdAdd(raw) {
  ...
  if(raw.startsWith('magnet:')){
    ...
    playlistAdd({type:'magnet',value:raw,name:dn||hm[1].slice(0,8)+'…'});
    return;
  }
  ...
  if(raw.startsWith('http')){
    ...
    playlistAdd({type:'url',value:raw,name:decodeURIComponent(parts[parts.length-1].split('?')[0])||'video'});
    return;
  }
...
function updateTrackerUI(sk,status,color){
  const el=document.getElementById('hs-tr-'+sk);
...
function archivePick(item) {
  ...
  fetch('https://archive.org/metadata/'+item.identifier)
    ...
    .then(function(meta){
      ...
      transition('BUFFERING');
      playDirect(url,[]);
      // FIX: removed stray transition('PLAYING') — onplaying handler does this
...
function cmdPick(n) {
  ...
  fetch(host+'/api/v1/videos/'+uuid)
    ...
    .then(function(data){
      ...
      transition('IDLE');startSwarmThenRelay(hash,ws||null,data.name);
    })
...
function dumpStatus() {
  ...
  if(S.torrent){
    ...
    if(t.wires&&t.wires.length){
      t.wires.forEach(function(w,i){
        log('  wire['+i+']:',w.type||'?',w.remoteAddress||w.id||'?',
          'dl:',fmtSpd(w.downloadSpeed?w.downloadSpeed():0),
          'ul:',fmtSpd(w.uploadSpeed?w.uploadSpeed():0));
      });
    } else {
      log('  no wires connected');
    }
  } else {
    log('torrent: null');
  }
...
function hookChat(){
  ...
  socket.on('chatMsg',function(d){
    ...
    const mk=body.match(/^magnet\s+(magnet:.+)$/i);
    if(mk){if(d.username===S.myName)cmdMagnet(mk[1]);return;}
...
function onCmd(sender,cmd,args,body){
  ...
  if(cmd==='_i'){
    ...
    log('_i hash='+hash+' ws='+(ws?ws.slice(0,60):'(none)'));
...
function cmdHelp(){
  chat('HiveStream v'+V+': top | search <q> | pick <n> | url <https://…> | magnet <uri> | pt <url> | <hash> | peers | add <…> | queue | next | clear | trackers | sync | seek <s> | stop | status');
}
function cmdStatus(){
  ...
  let rtc=0,ws=0;
  if(t.wires)t.wires.forEach(function(w){if(w.type==='webSeed')ws++;else rtc++;});
  chat(S.state+' · '+pct+'% · '+rtc+'🔗rtc '+ws+'🌐ws · '+fmtSpd(t.downloadSpeed)+'▼ '+fmtSpd(t.uploadSpeed)+'▲ | queue:'+q);
}
function cmdInfo(n){
